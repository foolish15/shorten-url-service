...
//Create create block
func (g *Gorm) Create(bl *schemas.Link) error {
	db := g.DB
...
func (g *Gorm) Update(bl *schemas.Link) error {
	db := g.DB
...
//Find find reward
func (g *Gorm) Find(queries ...repositories.Query) (bls []schemas.Link, err error) {
...
//First find reward
func (g *Gorm) First(queries ...repositories.Query) (bl schemas.Link, err error) {
	db := g.Concat(g.DB, queries...)
...
func (g *Gorm) Count(queries ...repositories.Query) (cnt int64, err error) {
	db := g.Concat(g.DB, queries...)
	err = db.Model(schemas.Link{}).Count(&cnt).Error
	if err != nil {
		return 0, err
	}
	return cnt, err
